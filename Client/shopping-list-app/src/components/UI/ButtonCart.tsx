...
export function ButtonCart() {
  ...
  const [quantity, setQuantity] = useState(dataList?.items.length)
  
  useEffect(() => {
    let totalItems = 0
    dataList?.items.forEach(currListItem => totalItems += currListItem.items.length)
    setQuantity(totalItems)
  }, [dataList])
  

  return (
    <a href="" data-item={quantity} className="a-buttoncart">
